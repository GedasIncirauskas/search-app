...
class SearchBar extends Component {
	...
	imageSearchHandler () {
		...
		axios.get(`/photos?query=${this.state.searchText}&client_id=${accessKey}`)
			.then(response => {
				this.setState({
	...
	searchAgain (item) {
		...
		axios.get(`/photos?query=${item}&client_id=${accessKey}`)
			.then(response => {
				this.setState({
					imagesBox: response.data.results
				});
		...
		return (
			<div>
				<div className='Input-style'>
					...
					{this.state.searchText.length > 0 && this.state.imagesBox.length > 0 ? 
						<button onClick={(e) => this.saveSearch(e)}>Save</button> : null
					}
				...
				{this.props.savedSearch.map((item, index) => (
					<span 
						key={index} 
						onClick={() => this.searchAgain(item, index)}>{item}
						<span 
							className='close-button' 
							onClick={() => this.props.deleteSearch(index)}>X
						</span>
					</span>
				))}
